...
describe.each(inject('withNodeWorkaround'))('using withNodeWorkaround: %s', (withNodeWorkaround) => {
  describe('parseURL, while mocking parse', () => {
    it.each([1, 2, 3, 10, 17, 18, 19, 20, 21, 1_000, undefined])('accepts chunk size of %s', async (chunkSize) => {
      ...
      let result = ''
      let bytes = 0
      for await (const { row, meta: { byteOffset, byteCount } } of parseURL(url, { chunkSize, lastByte, parse: parseMock })) {
        result += row
    ...
    it.each([
      ...
      revoke()
      if (lastByte === undefined && withNodeWorkaround) {
  ...
  describe('Papaparse high-level tests', () => {
    PARSE_TESTS.forEach((test) => {
      it(test.description, async () => {
        ...
        const result = []
        for await (const r of parseURL(url, { ...config, lastByte })) {
          result.push(r)
        }
        revoke()
  ...
  describe('parseURL', () => {
    ...
    it.each([1, 2, 3, 5, 9, 14])('with known delimiter and newline, accepts chunk size of %s bytes.', async (chunkSize) => {
      ...
      const result = []
      let bytes = 0
      for await (const { row, meta: { byteOffset, byteCount } } of parseURL(url, { chunkSize, lastByte })) {
        result.push(row)
        expect(byteOffset).toBe(bytes)
        bytes += byteCount
      }
    ...
    it.each([undefined, true, false])('should respect the stripBOM option (%s), and count the bytes correctly', async (stripBOM) => {
      ...
      const result = []
      for await (const r of parseURL(url, { stripBOM, lastByte })) {
        result.push(r)
      }
      revoke()
    ...
    it.each([
      ...
      expect(result).toEqual([{
        ...
        meta: {
          ...
          byteCount: lastByteAdjusted - expectedByteOffset + 1,
          charCount, // TODO(SL): define what is a "character" (UTF-16 code point? grapheme?)
          delimiter: ',',
          newline: '\n',
        },
      }])
      expect((result[0]?.meta.byteCount ?? -Infinity) + (result[0]?.meta.byteOffset ?? -Infinity)).toBe(lastByteAdjusted + 1)
    })
    ...
    it('should search invalid bytes over multiple chunks if needed', async () => {
      ...
      for await (const r of parseURL(url, { chunkSize: 1, firstByte: 1, lastByte, delimiter: ',', newline: '\n' })) {
        result.push(r)
      }
      revoke()
    ...
    it('should report invalid data in multiple rows', async () => {
      ...
      const result = []
      for await (const r of parseURL(url, { firstByte: 3, lastByte, delimiter: ',', newline: '\n' })) {
        result.push(r)
      }
      revoke()
    ...
    it('should handle empty Blob URLs', async () => {
      ...
      const result = []
      for await (const r of parseURL(url, { lastByte })) {
        result.push(r)
      }
      revoke()
    ...
    it('should return without any iteration if the Blob URL is empty', async () => {
      const url = URL.createObjectURL(new Blob(['']))
      const result = []
      for await (const r of parseURL(url)) {
        result.push(r)
      }
